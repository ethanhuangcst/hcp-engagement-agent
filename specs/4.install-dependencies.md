...
`npm install` 只安装 Node 依赖。**业务主数据**存 **远程 MySQL**（环境变量 `DATABASE_URL`），**不再**用本地 JSON 作 Twin / Options / 会话主库。RAG **语料源文件**与 **Qdrant 卷**仍可用仓库下 `data/rag`、`data/qdrant`（非主库）。

### 0.1 存储分工

| 资产 | 位置 | 写入方 |
|------|------|--------|
| Twin / Insights / Options / Sessions / 租户元数据 | **MySQL** | `hcp-twin-mcp`、BFF、Agent |
| ingest manifest 元数据 | **MySQL** | `medical-kb` |
...
| **主数据存储** | **MySQL** | 8.x+ | 已规划 | Twin / Insights / Options / 会话 |
| **DB 访问** | `mysql2` + Zod | — | 已装 | `packages/db`；**不用** Prisma |
...
├── .github/workflows/               # docker-build.yml 等
├── .cursor/skills/                  # 本仓 Agent Skills
├── package.json
├── package-lock.json
├── .npmrc
├── .env.example
├── docker-compose.yml               # 拉 GHCR 制品 + qdrant；MySQL 用远程托管
└── .dockerignore
```
...
| docker-compose / BullMQ / Playwright / LangGraph / Tremor | Compose 拉 GHCR 制品（web/mcp）+ qdrant；BullMQ 等后置 · 见 [`ToDo.md`](./ToDo.md) |
...
- 输出写入远程 **MySQL**（见 §7）。构建进度真相在 MCP **进程内队列**（非 `twin_build_runs` 表）。
...
| `get_twin` / `get_insights` | 读 MySQL Twin / Insights |
...
| `MCP_PORT` | 否 | 默认 `3200` |
| `DATABASE_URL` | 是 | MySQL 连接串（与 web / Agent 共用） |
...
- 主题语料可暂存 `data/rag/corpus/{academic,compliance,tenants}`，再 embed 入 Qdrant；manifest 元数据写 MySQL。
...
| 原则 | 说明 |
|------|------|
| **MySQL 主库** | Twin、Insights、Options、Session、租户、ingest manifest 元数据均在远程 MySQL |
| **Schema 校验** | 读写经 `@hca/domain` Zod；JSON 列存结构化文档，拒绝脏数据入库 |
...
| **向量分离** | 不做 MySQL 向量作主向量库（P0）；向量仅 **Qdrant** |

### 7.2 逻辑表（示意）

| 表 | 内容 | 迁移 |
|----|------|------|
| `hcp_twins` | `hcp_id` PK；`identity` / `twin` JSON；`tags`；`as_of`；`twin_version` | 001 |
| `hcp_insights` | `hcp_id` PK/FK；`payload` JSON（含 `doing_now`） | 001 |
| `ingest_manifest` / `rag_ingest_jobs` | 灌库清单与异步 job | 002 |
| `engagement_options` | `run_id`；`hcp_id`；方案 JSON；引用字段 | 003 |
| `chat_sessions` | `session_id`；`hcp_id`（`open_chat` → `_agent_general`）；`mode`；消息 JSON | 003–004 |
| `tenants` | 租户与 SOP 元数据 | 后置 |
| （可选）`twin_build_runs` | 构建进度持久化 | **未建**；现行 MCP 进程内 status |

字段名与 [`5.hcp-twin-data-dictionary.md`](./5.hcp-twin-data-dictionary.md) 对齐；实现为 SQL 迁移 + `mysql2`（非 Prisma）。
...
```bash
# .env（勿提交；密码须 URL 编码，例如 @ → %40）
DATABASE_URL=mysql://root:<PASSWORD>@38.55.199.241:3306/hca
```
...
| `revise_engagement` | 读/写 MySQL `chat_sessions`，写回修订 |
...
| `DATABASE_URL` | 是 | MySQL；与 web / MCP 共用 |
...
| 健康 | `GET /api/health` | MySQL + MCP +（可选）Qdrant |
...
- 健康检查：MySQL + MCP `health_check`（+ 可选 medical-kb / LLM）。

### 9.3 环境变量（Web 进程）

| 变量 | 必填 | 说明 |
|------|------|------|
| `DATABASE_URL` | 是 | MySQL |
...
[`docker-compose.yml`](../docker-compose.yml) 拉取 CI 推送到 GHCR 的应用镜像；Dockerfile 仅供 Actions 构建（[`docker/Dockerfile.web`](../docker/Dockerfile.web)、[`docker/Dockerfile.mcp`](../docker/Dockerfile.mcp)）。

| 服务 | 镜像来源 | 关键挂载 / 环境 |
|------|----------|-----------------|
| `web` | `ghcr.io/ethanhuangcst/hcp-engagement-agent/web:${IMAGE_TAG:-latest}` | `DATABASE_URL`、`MCP_URL=http://hcp-twin-mcp:3200`、`QDRANT_URL=http://qdrant:6333`；宿主机 `:3001` |
| `hcp-twin-mcp` | `ghcr.io/ethanhuangcst/hcp-engagement-agent/hcp-twin-mcp:${IMAGE_TAG:-latest}` | `DATABASE_URL`、`TWIN_MODE=live`；宿主机 `:3200` |
| `qdrant` | Docker Hub `qdrant/qdrant:v1.14.1` | `./data/qdrant`；**仅** `127.0.0.1:6333` |
| `redis` / `nginx` | 可选 · 后置 | 多实例队列 / TLS |

```bash
cp .env.example .env   # 填 DATABASE_URL
# 私有包：docker login ghcr.io
docker compose pull && docker compose up -d
# IMAGE_TAG=sha-…|semver|latest
```

CI：`.github/workflows/docker-build.yml`（PR 只 build；`main` / `v*` 推 GHCR）。完整步骤见 [`9.deploy.md`](./9.deploy.md)。

**MySQL 使用远程托管实例**（不在 compose 内默认起库）。
...
| 主存储 | PostgreSQL `mia` | **MySQL**（Twin / Options / 会话） |
