/**
 * One-shot: copy rows from legacy Postgres → MySQL.
 *
 * Env:
 *   SOURCE_DATABASE_URL  postgresql://…  (default: sibling project .env if unset)
 *   DATABASE_URL         mysql://…       (target; from repo .env)
 *
 * Usage: npm run migrate:pg-to-mysql -w @hca/db
 */
...
async function main(): Promise<void> {
  // Prefer repo .env for MySQL target (ignore inherited DATABASE_URL from shell/sibling).
  ...
  const sourceUrl =
    ...
    readEnvKey(join(ROOT, "../hcp-engagement-assistant/.env"), "DATABASE_URL") ??
    readEnvKey(join(ROOT, ".env.postgres"), "DATABASE_URL");

  if (!sourceUrl?.startsWith("postgres")) {
    throw new Error(
      "SOURCE_DATABASE_URL (postgresql://…) required for pg→mysql copy",
    );
  }
  if (!process.env.DATABASE_URL?.startsWith("mysql")) {
    throw new Error(
      "Target DATABASE_URL must be mysql://… (set in repo .env)",
    );
  }
  ...
  if (mig.status !== 0) throw new Error("MySQL migrate failed");

  const pgPool = new pg.Pool({ connectionString: sourceUrl });
  const mysql = getPool();

  const counts: Record<string, { src: number; dst: number }> = {};

  try {
    // Disable FK checks while bulk loading (order still parent-first)
    await mysql.query("SET FOREIGN_KEY_CHECKS = 0");

    for (const table of TABLES_ORDER) {
      const src = await pgPool.query(`SELECT * FROM ${table}`);
      console.log(`Copy ${table}: ${src.rowCount} rows`);

      // Clear target (except we may keep agent_general from migration — wipe all then re-insert)
      ...
      const dst = await mysql.query<{ c: number }>(
        `SELECT COUNT(*) AS c FROM ${table}`,
      );
      counts[table] = {
        src: src.rowCount ?? 0,
        dst: Number(dst.rows[0]?.c ?? 0),
      };
    }
    ...
    for (const [t, c] of Object.entries(counts)) {
      ...
      console.log(`  ${t}: pg=${c.src} mysql=${c.dst} [${mark}]`);
    }
