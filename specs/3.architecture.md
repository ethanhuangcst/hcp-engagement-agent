...
| 本地 JSON 作主库 | 规格选定 **香港 Postgres** + Qdrant（向量） |
...
| 共享主库 | web / twin-mcp / Agent / medical-kb 共用 `DATABASE_URL`（香港 Postgres） |
...
**Postgres 主机在香港机房**（见 [`4.install-dependencies.md`](./4.install-dependencies.md) §7）；**不**再使用 Twin/Options 本地 JSON 文件主库。
...
| 存储 | 内容 | 一致性 |
|------|------|--------|
| **Postgres（香港）** | Twin、Insights、Options、Session、租户、ingest manifest 元数据 | 事务 + 行级锁 / 业务互斥 |
...
| 数据驻留 | 业务主数据在**香港 Postgres**；向量在内网 Qdrant；语料暂存 `data/rag/corpus/`；**无默认公有云向量 SaaS** |
...
| ID | 决策 | 理由 | 放弃方案 |
|----|------|------|----------|
| ADR-01 | 主数据 **香港 Postgres**（JSONB + 关系表） | 多进程共享、可备份、可并发；替代本地 JSON 文件 | 本地 JSON 文件主库 |
